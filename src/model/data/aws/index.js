...
async function deleteFragment(ownerId, id) {
  ...
  // try {
  //   // Wait for the data to come back from AWS
  //   const data = await ddbDocClient.send(commandMeta);
  //   // We may or may not get back any data (e.g., no item found for the given key).
  //   // If we get back an item (fragment), we'll return it.  Otherwise we'll return `undefined`.
  //   return data?.Item;
  // } catch (err) {
  ...
  const commandMeta = new DeleteCommand(paramsMeta);
  ...
  try {
    ...
    logger.error({ err, params }, 'Unable to delete fragment data from S3');
    throw new Error('Unable to delete fragment data from S3');
  }

  try {
    // delete the metadata from the Amazon DynamoDB.
    await ddbDocClient.send(commandMeta);
  } catch (err) {
    logger.error({ err, paramsMeta }, 'Unable to delete fragment from DynamoDB');
